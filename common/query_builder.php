<?php
	...
	// a small function for selecting a single table with no condition
	...
	// a small function for selecting a single table with one condition
	...
	// a small function for selecting a table with two conditions
	...
	// a small function for selecting a table with three conditions
function selectSingleTableWithThreeCondition($conn, $table_name, $condition_one, $condition_two, $condition_three){
	...
	// a dinamic insert function for insterting data in a table
	...
	// A dinamice function for update data in a table with one condition
	...
	// A dinamic update function for update data in a table with two conditions
	...
	// A dinamice function for deleting a single row with no conditions
	...
	// A function for selecting user list And filtering for managemant
...
function allAtttendances($conn, $all_batchs){
	$sql = "SELECT attendances.id, attendances.user_id, attendances.date, attendances.rating, attendances.review, attendances.approved, batches.title, users.name
			FROM attendances
			LEFT JOIN batches ON attendances.batch_id = batches.id
			LEFT JOIN users ON attendances.user_id = users.id
			WHERE attendances.batch_id IN $all_batchs
			AND attendances.user_id != ".$_SESSION['user']['id']."
			ORDER BY attendances.date DESC";

	return $result = mysqli_query($conn, $sql);
}

function AlltrainerAttendances($conn, $get_data = null){
	if (empty($get_data)) {
		$sql = "SELECT users.name, attendances.id, attendances.date, attendances.rating, attendances.review, attendances.approved, batches.title
		FROM attendances
		JOIN users ON users.id = attendances.user_id
		JOIN batches ON batches.id = attendances.batch_id
		...
		$sql = "SELECT users.name, attendances.id, attendances.date, attendances.rating, attendances.review, attendances.approved, batches.title
			...
			if (array_key_exists('review', $get_data) && $get_data['review'] != '') {
						$condition[] = 'attendances.review LIKE "%'.$get_data['review'].'%"';
					}
...
function AllstudentAttendances($conn, $get_data = null){
	if (empty($get_data)) {
		$sql = "SELECT users.name, attendances.id, attendances.date, attendances.rating, attendances.review, attendances.approved, batches.title
		FROM attendances
		JOIN users ON users.id = attendances.user_id
...
function AlltrainersPendingAttendances($conn){
	$sql = "SELECT users.name, attendances.id, attendances.date, attendances.rating, attendances.review, batches.title
		FROM attendances
		JOIN users ON users.id = attendances.user_id
		JOIN batches ON batches.id = attendances.batch_id
		WHERE users.role_id = 2
...
function AllstudentsPendingAttendances($conn){
	$sql = "SELECT users.name, attendances.id, attendances.date, attendances.rating, attendances.review, batches.title
		FROM attendances
		JOIN users ON users.id = attendances.user_id
		JOIN batches ON batches.id = attendances.batch_id
